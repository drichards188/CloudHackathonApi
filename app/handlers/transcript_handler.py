...
class TranscriptHandler:
    @staticmethod
    def handle_get_transcript_results(event, context):
        print("---> lambda_handler is running")
        try:
            symbol = event["symbol"]
            expression = event["expression"]
            transcript_response = get_transcript(symbol.upper(), expression)
            changes = []
            percent_changes = []
            ...
            return {"symbol": symbol, "expression": expression, "changes": changes, "percent": percent_changes,
                    "dates": transcript_response}
...
def get_from_market_data(symbol: str, start_date: str, end_date: str):
    ...
    if symbol:
        ...
        api_key = os.getenv("MARKET_DATA_KEY")
